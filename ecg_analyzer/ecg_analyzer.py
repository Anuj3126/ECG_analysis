...
from .signal_processing import SignalProcessor, SignalInterpreter
...
class ECGAnalyzer:
    ...
    def analyze_ecg(self, image_path: str, extract_signals: bool = True, interpret_signals: bool = True) -> CompleteECGAnalysis:
        """Perform complete ECG analysis including text extraction, signal processing, and interpretation."""
        ...
        signal_data = None
        signal_interpretation = None
        if extract_signals:
            print("  → Processing ECG signals...")
            try:
                signal_data = self.signal_processor.process_ecg_image(image_path)
                
                # Interpret signals (optional)
                if interpret_signals and signal_data.leads:
                    print("  → Interpreting ECG signals...")
                    try:
                        signal_interpretation = self.signal_interpreter.interpret_all_signals(signal_data)
                    except Exception as e:
                        print(f"  → Signal interpretation failed: {e}")
                        signal_interpretation = None
        ...
        analysis = CompleteECGAnalysis(
            ...
            analysis_metadata={
                ...
                "signal_interpretation_successful": signal_interpretation is not None,
